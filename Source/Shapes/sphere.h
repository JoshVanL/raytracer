...
public:
    ...
    Sphere(
        ...
        float &refl,
        float &transp,
        glm::vec3 &ec) : radius(r), radius2(r * r), center(c), surfaceColor(sc), emissionColor(ec), transparency(transp), reflection(refl)
    {   
        
    };
    Sphere(glm::vec3 &c, float &r, glm::vec3 &sc) : reflection(0), transparency(0), emissionColor(vec3(0,0,0)) {
        
    };

    bool intersect(const Ray &ray, glm::vec3 dir, Intersection& intersection)
    {
        ...
        vec4 intersectPoint(temp.x, temp.y, temp.z, 1);
        float d = glm::distance(intersectPoint, ray.position);
        Intersection sphereIntersection(intersectPoint, d, this);
